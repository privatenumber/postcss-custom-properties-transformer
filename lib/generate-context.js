...
class Context {
	constructor(filepath, css, hashSalt = '') {
		Object.assign(this, {
			filepath,
			css,
			hashSalt,
	...
	hash() {
		...
		let hashed = crypto.createHash(hashType).update(this.hashSalt + this[key]).digest(digestType);
...
function generateContext({file, css}, hashSalt) {
	const context = new Context(file, css, hashSalt);
	return propertyName => {
